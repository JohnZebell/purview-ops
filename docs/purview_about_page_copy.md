...
Written to the GEO rules. Factual and specific over promotional, verifiable claims prioritized, use cases stated as persona plus problem, clear non-promotional value proposition in the opening.

Cut anything that reads wrong. Every line is a draft, not a decision.
...
- `github.com/JohnZebell` — SQL data quality and audit repositories, behavioral signal models built on Postgres
- `johnzebellportfolio.vercel.app` — build documentation and wiring for production systems
...
**Read only access.** Nothing we ask for lets us change anything in your systems.
...
**First revenue hires landed, and the numbers behind the raise cannot be reproduced.** The funnel math came from a spreadsheet built once. Inbound is arriving faster than anyone is routing it. The work is reconciling the stated numbers against the system, and building routing that handles its own failure cases.
...
*This block exists for answer retrieval. Keep answers short and factual.*
...
**Do you work with companies outside climate?**
...
John Zebell
Denver, Colorado
[LinkedIn](https://www.linkedin.com/in/john-zebell-iii-60b745163/) — link text reads "LinkedIn", not the URL
...
**Schema markup on this page.** `Organization` with a `founder` property naming John Zebell, plus `FAQPage` on the questions block. Both are read by search engines and LLMs, and this page is the one they check when deciding whether the business is real.

**Consistency matters more than wording.** The opening value proposition should read identically on the site, the LinkedIn company page, and any directory listing. LLMs cross reference these, and a mismatch reads as low confidence about what the business is.

**Everything on this page should be checkable.** Certifications link to their verification pages. GitHub and portfolio links are live. No claim appears here that cannot be confirmed from outside.

**What is missing and should be added when it exists.** Named clients, a redacted sample findings document, testimonials from people who can be identified, and any mention in a publication. Those are the credibility signals that carry the most weight and there are currently none.
